...
func HTML2Text(htmlDocument string) (string, error) {
	...
	f = func(n *html.Node, activeBuilder *strings.Builder) {
		...
		case html.TextNode:
			...
			// the prev node has a new line, so it is safe to trim the indentation
			if !canAddNewLine {
		...
		case html.ElementNode:
			if n.Data == "br" {
				...
				canAddNewLine = false
			} else if canAddNewLine && !slices.Contains(inlineTags, n.Data) {
				activeBuilder.WriteString("\r\n")
				canAddNewLine = false
			}
		...
		for c := n.FirstChild; c != nil; c = c.NextSibling {
			if c.Type != html.ElementNode || !slices.Contains(tagsToSkip, c.Data) {
				f(c, activeBuilder)
			}
		}
